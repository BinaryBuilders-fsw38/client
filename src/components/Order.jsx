        ...
        if (!user_id) {
          ...
          try {
            const response = await axios.get(
              `${apiUrl}/checkout/getByUser/${user_id}`
            );
  ...
  const handlePaidStatus = (status) => {
    ...
    if (flag) {
      ...
      if (
        ...
        filteredSelectedDateOrder = Array.isArray(tempState)
          ? tempState.filter(
              (item) =>
                item.payment_status.toLowerCase() === status.toLowerCase()
            )
          : [];
      }

      setSelectedDateOrder(
        filteredSelectedDateOrder !== undefined ? filteredSelectedDateOrder : []
      );
      ...
      if (status.toLowerCase() === "paid") {
        setFilteredOrder(
          dataOrder.filter(
            (item) => item.payment_status.toLowerCase() === status.toLowerCase()
          )
        );
  ...
  return (
    <div>
      ...
      <div className="max-w-5xl py-3 rounded-md bg-white border border-slate-300 max-h-full mx-auto">
        <div className="max-w-full xl:flex lg:flex md:flex sm:flex gap-3 items-center">
          <SearchComponent />
        ...
        <div className="max-w-full flex gap-3 items-center ml-16 my-4">
          ...
          <button
            className={`border-2 ${
              activeStatus === "all" ? "bg-slate-900 text-white" : "bg-white"
            } rounded-lg font-semibold  px-2 py-1 text-sm`}
            onClick={() => {
          ...
          <button
            className={`border-2 ${
              activeStatus === "Paid" ? "bg-slate-900 text-white" : "bg-white"
            } rounded-lg font-semibold px-2 py-1 text-sm`}
            onClick={() => {
          ...
          <button
            className={`border-2 ${
              activeStatus === "Unpaid" ? "bg-slate-900 text-white" : "bg-white"
            } rounded-lg font-semibold px-2 py-1 text-sm`}
            onClick={() => {
        ...
        {dataOrder.length === 0 ? (
          <div className="text-center font-bold">
            You don't have any checkouts yet
          </div>
        ) : (
          <>
            <CardOrder
              dataOrder={flag ? selectedDateOrder : filteredOrder || dataOrder}
            />
            <CardOrderTrack />
          </>
        )}
